...
function updateQty(pid,size,color,attr_id,price){
  ...
  jQuery('#qty').val(qty)
  add_to_cart(pid,size,color);
  jQuery('#total_price_'+attr_id).html('PKR '+qty*price);
}
...
function calculatePlaninstalmentselecter()
{
  var downPayment = jQuery("#downPayment").val();
  var totalAmountPayable = jQuery("#totalAmountPayable").val();
  var radioValue =jQuery("input[name='installmentPlan']:checked").val();
  var remainingAmount = (totalAmountPayable-downPayment);
  
  document.getElementById("resultAmount").innerHTML= "Rs. "+(remainingAmount/radioValue).toFixed(0)+"/- only";
  

}


jQuery('#payment-installments-form').submit(function(e){
  e.preventDefault();
  jQuery.ajax({
    url:'/add_installement',
    headers: {
      'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')
    },
    data:jQuery('#payment-installments-form').serialize(),
    type:'post',
    success:function(result){
      if(result.status=="success"){
        
        jQuery('.msg').show().html(result.msg);
        jQuery('#payment-installments-form')[0].reset();
        fetch_data();
...
function fetch_data()
      {
        ...
        jQuery.ajax({
        type:'get',
        url:'/fetch_installments/'+order_id,
        success:function(result){
            
            jQuery('#installment_list').html(result);
        }
        });
      }
